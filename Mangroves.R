...
# Evaluate PAs' role in mangrove conservation. ----------------------------
# Process the PA data.
...
# Generate the mean distance to other sample points.
# Calculate the distance matrix (the following part can be replaced by makeTiles function)
...
write.csv(mangrove_standardized, "./Data/Mangrove_match_processed.csv", row.names = F)
...
ggsave("./Figures/plot_coef_drivers.jpg", plot_coef_drivers, device = "jpeg", width = 4.55, height = 4, units = "cm", dpi = 300)
...
ggsave("./Figures/plot_coef_pa.jpg", plot_coef_pa, device = "jpeg", width = 4.55, height = 4, units = "cm", dpi = 300)
...
ggsave("./Figures/interact_pa_rq.jpg", interact_pa_rq, device = "jpeg", width = 4.55, height = 4, units = "cm", dpi = 300)
...
ggsave("./Figures/v_test.jpg", v_test_plot, device = "jpeg", width = 14, height = 9, dpi = 300, units = "cm")

# Unused fitting of GLMMs. ------------------------------------------------
...
mod_f <- as.formula(loss ~ gdp  + hf  + pop  + city + road + PA + 
                      coconut + coffee + olp + rice + rubber + sugarcane + 
                      loss_prev + 
                      river + coastline + typhoon + sealevel +
                      rq)

rf = ranger(mod_f, data = train_mangrove, num.trees = 300, mtry = 18/3, importance = 'impurity', verbose = T)
...
ggsave("./Figures/fig_1.jpg", fig_1, device = "jpeg", width = 14, height = 6, dpi = 300)
...
for (i in 1:length(var_names))
{
  fig <- ggplot(results_sensitivity[[var_names[i]]]) +
    ...
    theme(axis.text = element_text(size = 20),
          ...
          panel.grid.major = element_line(color = 'black', linewidth = 0.1, linetype = 2))
  ggsave(paste0("./Figures/sensitivity_analysis/sensitivity_analysis_", var_names[i], ".jpg"), fig, device = "jpeg", width = 6, height = 6, dpi = 300)
...
# Future prediction under SSPs.----------------------------------------------
# Import prediction datasets.
pred_sets <- list.files("./Data/pred_sets", full.names = F)
Mangrove_Pred <- function(pred_set, mod)
{
  pred_ras <- rast(paste0("./Data/pred_sets/", pred_set))
  pred_ras <- predict(pred_set, mod, na.rm = T)
  writeRaster(pred_ras, paste0("./Data/pred_results/", pred_set))
  print(paste0("Prediction for ", pred_set, " is done."))
}
lapply(pred_sets, Mangrove_Pred, mod = rf)

#Data extract by country and continent

ras_files <- list.files("./Data/pred_results", full.names = T)
country = vect("")
Extract_V <- function(ras)
{
  ras <- rast(ras)
  extracted_values <- extract(ras, country, fun = mean) %>%
    as.data.frame()
  colnames(extracted_values) <- c("ID", paste0("Value_", basename(raster_file)))
  print(paste0("Extraction for ", ras, " is done."))
  return(extracted_values)
}
results <- lapply(ras_files, Extract_V) %>% 
  do.call(merge, ., args = list(by = "ID", all = T))

write.csv(results, "./Data/extracted_values.csv", row.names = F)
